...
void DijkstrasPathfindingSystem::drawDebug(sf::RenderWindow &window, const TileMap &map) {
    const float size = static_cast<float>(map.getTileSize());

    sf::RectangleShape cell(sf::Vector2f(size, size));
    cell.setFillColor(sf::Color(255, 0, 0, 80));
    ...
    cell.setFillColor(sf::Color(255, 255, 0, 80));
